...
const authorizationSchemeSchema = z.string().regex(
  /^[A-Za-z][A-Za-z0-9._~-]*$/,
  'Authorization scheme must be an RFC token such as "Bearer" or "Basic"',
)

export const AuthorizationHeaderCredentialConfigSchema = z
  .object({
    host: domainPatternSchema.describe(
      'Destination host for swap-on-access injection',
    ),
    scheme: authorizationSchemeSchema.describe('Authorization scheme'),
    value: z
      .string()
      .min(1)
      .describe('Resolved credential value held in host memory only'),
  })
  .strict()

const credentialPlaceholderValueSchema = z
  .string()
  .regex(
    /^keelcred_[A-Za-z0-9._~-]+$/,
    'Credential placeholder must be a generated keelcred_* sentinel',
  )

export const AuthorizationPlaceholderCredentialConfigSchema = z
  .object({
    host: domainPatternSchema.describe(
      'Destination host where this placeholder may be swapped',
    ),
    scheme: authorizationSchemeSchema.describe('Authorization scheme'),
    placeholder: credentialPlaceholderValueSchema.describe(
      'Sandbox-visible placeholder value',
    ),
    value: z.string().min(1).describe('Resolved credential value held in host memory only'),
  })
  .strict()
...
export const CredentialsConfigSchema = z
  .object({
    ...
    authorizationHeaders: z
      .array(AuthorizationHeaderCredentialConfigSchema)
      .optional()
      .describe(
        'Host-memory credentials to inject as Authorization headers when the destination host matches. The sandboxed process does not receive these values.',
      ),
    authorizationPlaceholders: z
      .array(AuthorizationPlaceholderCredentialConfigSchema)
      .optional()
      .describe(
        'Host-memory credentials to substitute when the sandbox sends a bound keelcred_* placeholder in Authorization.',
      ),
...
export const SandboxRuntimeConfigSchema = z
  ...
  .superRefine((cfg, ctx) => {
    ...
    let hasAuthorizationHeaders = false
    ...
    for (const [idx, v] of (creds.authorizationHeaders ?? []).entries()) {
      hasAuthorizationHeaders = true
      checkSubset([v.host], [
        'credentials',
    ...
    if (
      (hasMaskedEnv || hasAuthorizationHeaders) &&
...
>
export type AuthorizationHeaderCredentialConfig = z.infer<
  typeof AuthorizationHeaderCredentialConfigSchema
>
