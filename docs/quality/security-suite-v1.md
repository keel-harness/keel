...
Epic 3.22 adds a separately gated, backend-specific connect-time address guard for
`srt:vendored` TCP. Its product, carrier, real-sandbox, exception, resolver, audit, and teardown
evidence strengthens the behavior behind SEC-003 without changing the catalog row to `PASS`.
SEC-003 and SEC-015 remain `DOC-LIMIT`; alternate backends, provider API egress, UDP/QUIC, and
CONNECT/SNI equivalence are not covered by that implementation.
...
| SEC-003 | DOC-LIMIT (not pass) | `packages/warden/src/egress-profile.test.ts`; `vendor/sandbox-runtime/test/sandbox/destination-dial.test.ts`; `vendor/sandbox-runtime/test/sandbox/destination-guard-proxy.test.ts` | Epic 3.22 implements a separately gated connect-time address guard for `srt:vendored` TCP, with product, carrier, real-sandbox, resolver, exception, audit, and teardown evidence under `pnpm test:egress-product` and `pnpm test:sandbox:real`. This row is not counted as pass: the catalog remains `DOC-LIMIT` because the implementation is not generic DNS-rebinding coverage for alternate backends, provider API calls, UDP/QUIC, or proxy-unaware traffic. |
...
| SEC-027 | PASS | `packages/warden/src/credential-proxy.test.ts`; `vendor/sandbox-runtime/test/sandbox/linux-proxy-readiness.test.ts`; `packages/warden/src/srt-sandbox.test.ts`; `packages/warden/src/srt-runtime-loader.test.ts`; `packages/warden/src/rpc-server.test.ts`; `packages/warden/src/capability-manifest.test.ts`; `packages/kernel/src/warden/runtime.test.ts`; `packages/kernel/src/cli/doctor.test.ts` | Governed-bash secretless egress credential proxy is proven for swap-on-access injection, placeholder host-binding, wrong-host and unknown-placeholder 403 leak guard, bounded fail-closed sandbox-proxy startup, fail-closed source resolution, source-file deny-read, and never-serialize invariants over sandbox env/argv/profile/policy/audit/response/status surfaces. Source power is provenance-scoped: **operator** config (`KEEL_WARDEN_CREDENTIAL_PROXY_RULES`) keeps env/file/absolute-command sources; the model-writable **project** file (`.keel/credential-proxy.json`) is forwarded on a separate channel and restricted to a realpath-contained workspace `file` source (command/env rejected, `~`/out-of-workspace rejected), and governed tools cannot write `<workspace>/.keel` at all. Not counted as all-tool governance, real-model product-path governance, keychain storage, signed/offline evidence, CONNECT/SNI hardening, or provenance-taint enforcement. |
