...
*The full revision history — what changed in each version from v1.1 onward, and why — is in*
*[`docs/spec-history.md`](docs/spec-history.md).*
...
1. **Tool executions are OS-sandboxed by default.** Filesystem: allow-only writes (workspace + declared temp), deny-then-allow reads (secrets paths denied by default). Network: namespace removed (Linux) / Seatbelt rules (macOS); supported governed TCP egress traverses the warden-controlled proxy enforcing a domain allowlist. On the active `srt:vendored` backend, the warden also resolves every destination immediately before a new connection, rejects the entire answer set if any address is denied or not narrowly excepted, and pins the vetted set to the final socket dial (ADR-0086). Engine: vendored `@anthropic-ai/sandbox-runtime` (Apache-2.0).
...
- **Resolved-address enforcement is backend-specific.** The `srt:vendored` TCP path has a warden-owned connect-time guard: it freshly resolves the requested destination, classifies every answer, denies mixed or unsafe sets, and gives SRT only the vetted addresses while retaining the original hostname for HTTP Host, TLS certificate verification, and SNI. This does not establish a generic DNS-rebinding guarantee for provider API calls, UDP/QUIC, proxy-unaware traffic, interactive-console guest activity, or alternate `SandboxPort` backends. SEC-003 therefore remains `DOC-LIMIT`; SEC-015 CONNECT/SNI equivalence remains separate and `DOC-LIMIT`.
...
- **An in-tree keel install is reduced-enforcement.** Every guarantee above assumes keel's own executable bytes live *outside* the workspace it is pointed at. That holds for the supported install paths — `npx keel-harness` and a global install both land in a cache/store directory the sandbox `allowWrite` root does not cover. Two layouts break the assumption: keel as a **local devDependency** (`<workspace>/node_modules/keel-harness/…`) and **self-hosting** (the keel repo *is* the workspace). In both, the warden entry, the vendored sandbox runtime, and the bundled ripgrep sit under the workspace root — inside `allowWrite`, protected by no `denyWrite` token — so a governed write can replace the code that evaluates policy, owns the sandbox profile, and writes the audit chain, persistently across sessions. keel does **not** defend against this: deny-writing those paths would break ordinary package installs, and when the workspace *is* the harness source, editing it is the task. `keel doctor` reports the posture (`harness-outside-workspace`) with the remedy; per §3.4 that is honest absence rather than theater. The mitigation is installation posture — do not install keel inside the workspace you point it at.
...
or roadmap text, not user install instructions.
...
- **OQ-1 (resolved 2026-07-09):** product `keel`; npm `keel-harness`; GitHub
  `keel-harness/keel`; canonical domain `keel-harness.com` (see REL-001). Package publication is
  done for the `0.1.1` pre-alpha carrier; further releases remain release tasks, not naming
  questions.
