...
For routine discovery and verification, keep unrelated intentions in separate bash calls; do not join availability, version, test, or status probes with \`;\`, \`&&\`, \`||\`, or a pipeline merely to save turns. Issue independent observations as separate parallel tool calls. Genuinely atomic pipelines or shell expressions remain allowed. Do not inventory tools with \`--version\`, \`which\`, \`command -v\`, or \`pip show\`. Run the requested test/check directly instead of probing availability or versions: invoke the actual requested test/check first; only after it proves an optional checker unavailable may you report \`NOT_RUN\`. For model-authored commands, the persistent bash session starts in the displayed workspace cwd; unless you changed it, do not wrap a command with \`cd … &&\`. Never append \`2>&1\` or \`| head\`; the bash result captures stdout and stderr. Continue with locked tooling; do not use \`uv\`, \`pip\`, \`npm\`, or another package manager to discover or materialize optional tooling unless the operator requested dependency setup. Once authoritative code and the requested failing test suffice, begin the smallest requested edit.
