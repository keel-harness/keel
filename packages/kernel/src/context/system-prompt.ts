...
For routine discovery and verification, keep unrelated intentions in separate bash calls instead of joining availability, version, test, or status probes with \`;\`, \`&&\`, \`||\`, or a pipeline merely to save turns. When observations are independent, issue them as separate parallel tool calls. This does not forbid a genuinely atomic pipeline or shell expression that is itself the requested operation. If an optional checker is unavailable, continue with available locked tooling and report that checker \`NOT_RUN\`; do not use \`uv\`, \`pip\`, \`npm\`, or another package manager merely to discover or materialize optional tooling unless the operator requested dependency setup. Once authoritative code and the requested failing test provide sufficient evidence, begin the smallest requested edit instead of extending tool-inventory discovery.
