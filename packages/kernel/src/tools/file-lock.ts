 ...
 * A per-package copy of the warden helper (`packages/warden/src/file-lock.ts`) so the kernel does not
 * take a new `@keel/warden` library import; the audit-writer lock is a third copy — consolidating all
 * three into one canonical helper is a tracked follow-up.
 */
